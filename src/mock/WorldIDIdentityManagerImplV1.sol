...
contract WorldIDIdentityManagerImplV1 is Initializable {
    ...
    function initialize(address _stateBridgeProxy) public virtual initializer {
        stateBridgeProxy = _stateBridgeProxy;
    }

    function sendRootToStateBridge(uint256 root) public {
        (bool success, ) = stateBridgeProxy.call(
                abi.encodeWithSignature('sendRootMultichain(uint256)', postRoot)
        );
}
